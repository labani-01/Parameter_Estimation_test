...
workflow = wf.Workflow(opts, "test_workflow")
...
for i, (priors, post_files, data_files) in enumerate(zip(opts.prior_catalog, opts.posterior_files, opts.data)):
    ...
    #hp_base, hc_base  = get_td_waveform(approximant="IMRPhenomXPHM", mass1=np.min(m1), mass2=np.min(m2), f_lower=20, delta_t=1.0/2048)
    ...
    start = hp_base.start_time
    print(start, int(np.min(m1) - 1), int(np.min(m2) - 1))
    end = hp_base.end_time
    print(int(start) - 30)
