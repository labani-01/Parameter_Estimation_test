...
workflow = wf.Workflow(opts, "ground_based_obs_nliv")
...
""")
    ...
    inference_file = node.new_output_file_opt(workflow.analysis_time, '.hdf', '--output-file', tags=[os.path.splitext(os.path.basename(priors))[0].replace("inference-", "nliv")])
    workflow += node
    node = demarg_exe.create_node()
    node.add_input_opt("--input-file", inference_file)
    demarg_file = node.new_output_file_opt(workflow.analysis_time, '.hdf', '--output-file', tags=[os.path.splitext(os.path.basename(priors))[0].replace("inference-", "dnliv")])
    workflow += node
    node = plot_exe.create_node()
    node.add_input_opt("--input-file", demarg_file)
    plot_file = node.new_output_file_opt(workflow.analysis_time, '.png', '--output-file', tags=[os.path.splitext(os.path.basename(priors))[0].replace("inference-", "pnliv")])
    workflow += node
workflow.save()
